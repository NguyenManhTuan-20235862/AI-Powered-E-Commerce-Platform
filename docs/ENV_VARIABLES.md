...
| `LLM_BASE_URL` | `app/core/config.py` → `app/core/llm.py` (`ChatOpenAI`, task 6.1.1) | Endpoint LLM tương thích OpenAI - dev trỏ Ollama local (`http://host.docker.internal:11434/v1`), production để trống = dùng endpoint OpenAI mặc định. **PHẢI đổi khi deploy** nếu không dùng OpenAI mặc định. |
| `LLM_API_KEY` | `app/core/config.py` → `app/core/llm.py` | API key cho LLM - dev dùng Ollama không check giá trị này (để chuỗi giả bất kỳ, VD `ollama`), production **PHẢI đổi thành API key OpenAI thật**. |
| `LLM_MODEL` | `app/core/config.py` → `app/core/llm.py` | Tên model - dev `llama3.2` (Ollama), production **PHẢI đổi** sang model OpenAI thật (VD `gpt-4o-mini`). |
| `LLM_MAX_TOKENS` | `app/core/config.py` → `app/core/llm.py` | Giới hạn số token mỗi response - bảo vệ chi phí khi dùng OpenAI thật, default `1000`. |
| `LLM_TIMEOUT` | `app/core/config.py` → `app/core/llm.py` | Giây chờ tối đa mỗi lần gọi LLM trước khi coi là thất bại - default `30`. |
...
| `VNPAY_TMN_CODE` | `app/core/config.py` → `app/services/payment_service.py` | Mã merchant VNPay (đăng ký tại `sandbox.vnpayment.vn` cho dev) - rỗng = `POST /payments/create` trả `503`. **PHẢI đổi khi deploy** (tài khoản merchant thật, khác sandbox). |
| `VNPAY_HASH_SECRET` | `app/core/config.py` → `app/services/payment_service.py` | Secret ký/xác minh chữ ký HMAC-SHA512 cho request tạo giao dịch VÀ callback - **rủi ro bảo mật cao nếu lộ** (ai có secret này ký được callback giả, cùng mức độ nghiêm trọng `JWT_SECRET_KEY`). **PHẢI đổi khi deploy.** |
| `VNPAY_PAY_URL` | `app/core/config.py` → `app/services/payment_service.py` | URL cổng thanh toán VNPay - default endpoint sandbox v2.1.0. **PHẢI đổi sang endpoint production thật khi deploy.** |
| `VNPAY_RETURN_URL` | `app/core/config.py` → `app/services/payment_service.py` | URL Backend đầy đủ cho `GET /payments/callback` - PHẢI là URL trình duyệt KHÁCH gọi được (cùng nguyên tắc `NEXT_PUBLIC_API_URL`, KHÔNG PHẢI tên service Docker). **PHẢI đổi khi deploy.** |
| `FRONTEND_BASE_URL` | `app/core/config.py` → `app/services/payment_service.py`, `app/routers/payment.py` | URL gốc Frontend - Backend dùng để redirect khách về `/checkout/payment-result` sau khi xử lý callback thanh toán. **PHẢI đổi khi deploy.** |
